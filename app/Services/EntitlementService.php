...
class EntitlementService
{
    ...
    public function enforcementEnabledFor(Company $company): bool
    {
        return $company->subscription_enforcement_enabled !== null
            ? (bool) $company->subscription_enforcement_enabled
    ...
    public function readOnly(Company $company): bool{return $this->enforcementEnabledFor($company) && !$this->active($company);}
    public function feature(Company $company,string $feature): bool { if(!$this->enforcementEnabledFor($company))return true; $s=$this->current($company); if(!$s||!$s->ends_at->isFuture())return false; if($feature==='promo_codes'&&!array_key_exists($feature,$s->snapshot['features']??[]))return (int)($s->snapshot['rank']??-1)>=3; return (bool)($s->snapshot['features'][$feature] ?? false); }
    ...
    public function canAdd(Company $company,string $resource): bool { if(!$this->enforcementEnabledFor($company))return true; $s=$this->current($company); if(!$s||!$s->ends_at->isFuture())return false; $usage=$this->usage($company); $map=['company'=>'companies','user'=>'users','product'=>'products']; $key=$map[$resource]??null; if(!$key)return false; $limit=$key==='companies'?'company_limit':($key==='users'?'user_limit':'product_limit'); return $usage[$key] < (int)($s->snapshot[$limit]??0); }
    public function canAddUser(Company $company,int $userId): bool { if(!$this->enforcementEnabledFor($company))return true; $account=$company->subscriptionAccount; if(!$account)return false; $ids=$account->companies()->pluck('id'); if(CompanyUser::whereIn('company_id',$ids)->where('user_id',$userId)->where('status','active')->exists())return true; return $this->canAdd($company,'user'); }
    ...
    {
        if (!$this->enforcementEnabledFor($company)) return;
