...
class CategoryController extends Controller
{
    ...
    public function destroy(string $id)
    {
        ...
        if ($Object->status == 1) {
            ...
            // Aucun produit => suppression définitive
            if ($Object->products()->count() == 0) {

                Action::create([
                    'user_id' => auth()->user()->id,
                    'function' => 'SUPPRESSION D\'UNE CATEGORIE',
                    'text' => auth()->user()->name." a supprimé la catégorie : ".$Object->name,
                ]);
                ...
                return response()->json([
                    ...
                    "msg" => "La catégorie ".$categoryName." a été supprimée définitivement car elle ne contient aucun produit."
                ]);
            }

            // Contient des produits => archivage
            $Object->update([
                'status' => 0,
            ]);
            ...
            return response()->json([
                ...
                "msg" => "La catégorie ".$Object->name." a été archivée car elle contient des produits."
            ]);
        }
