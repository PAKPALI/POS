...
class ReconcileKprimePayPayments extends Command
{
    ...
    public function handle(KprimePayService $kprimePay, QuotaPaymentSettlementService $settlement): int
    {
        ...
        $this->info($prefix."analysés: {$payments->count()}, payés: {$counts['paid']}, échoués: {$counts['failed']}, expirés: {$counts['expired']}, inchangés: {$counts['unchanged']}, erreurs: {$counts['errors']}.");
