...
class MarketingSiteTest extends TestCase
{
    ...
    {
        $this->get('/')->assertOk()->assertSee('Vendez simplement. Gardez le contrôle.')->assertSee('Testez Maxanou dans les conditions réelles de votre commerce.')->assertSee('Maxanou en chiffres')->assertSee('Abonnements actifs')->assertSee('Statistiques agrégées et anonymisées')->assertSee(route('marketing.register'), false)->assertSee(route('marketing.login'), false)->assertSee('data-marketing-appearance', false)->assertSee('data-marketing-mode="light"', false)->assertSee('data-marketing-accent="#3B82F6"', false);
        $this->get('/partenaires')->assertOk()->assertSee('Le programme en chiffres')->assertSee('Partenaires actifs')->assertSee('Retraits effectués')->assertSee('aucun nom, contact ou montant individuel', false);
