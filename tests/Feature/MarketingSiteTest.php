...
class MarketingSiteTest extends TestCase
{
    ...
    {
        $this->get('/')->assertOk()->assertSee('Accueil')->assertSee('Vendez simplement. Gardez le contrôle.')->assertSee('Testez Maxanou dans les conditions réelles de votre commerce.')->assertSee('Maxanou en chiffres')->assertSee('Abonnements actifs')->assertSee('contact@maxanou.com')->assertSee('+228 79 68 51 06')->assertSee('tel:+22879685106', false)->assertSee('marketing-illustration-dashboard', false)->assertSee(route('marketing.register'), false)->assertSee(route('marketing.login'), false)->assertSee('data-marketing-appearance', false)->assertSee('data-marketing-mode="light"', false)->assertSee('data-marketing-accent="#3B82F6"', false);
        ...
        $this->get('/aide')->assertSee('mailto:contact@maxanou.com', false)->assertSee('Notre équipe reste disponible.');
    }
