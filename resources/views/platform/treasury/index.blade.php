...
<div class="platform-page-stack platform-treasury-page">
    ...
    <section class="platform-card platform-data-panel" aria-labelledby="withdrawal-history-title">
        ...
        <div class="table-responsive"><table class="table table-dark table-hover align-middle"><thead><tr><th>Référence</th><th>Bénéficiaire</th><th>Montant demandé</th><th>Frais KPrimePay</th><th>Statut</th><th>Date</th></tr></thead><tbody>@forelse($withdrawals as $withdrawal)
            @php($variant = $withdrawal->status === 'succeeded' ? 'success' : (in_array($withdrawal->status, ['failed', 'unknown']) ? 'danger' : 'warning'))
            <tr><td><small>#{{ $withdrawal->id }} · {{ $withdrawal->transaction_id }}</small><br><small class="text-secondary">{{ $withdrawal->kpp_reference ?: 'Référence KPrimePay en attente' }}</small></td><td>{{ $withdrawal->account_snapshot['beneficiary_name'] ?? '—' }}<br><small class="text-secondary">{{ $withdrawal->account_snapshot['gateway'] ?? '' }} · {{ $withdrawal->account_snapshot['phone'] ?? '' }}</small></td><td>{{ number_format($withdrawal->amount, 0, ',', ' ') }} XOF</td><td>{{ number_format($withdrawal->status === 'succeeded' ? $withdrawal->fees : $withdrawal->estimated_fees, 0, ',', ' ') }} XOF</td><td><span class="platform-status-chip is-{{ $variant }}"><i class="bi bi-circle-fill" aria-hidden="true"></i> {{ ['otp_verified' => 'À envoyer', 'processing' => 'En cours', 'unknown' => 'À vérifier', 'succeeded' => 'Succès', 'failed' => 'Échoué'][$withdrawal->status] ?? $withdrawal->status }}</span>@if($withdrawal->failure_reason)<br><small class="text-danger">{{ $withdrawal->failure_reason }}</small>@endif</td><td>{{ $withdrawal->requested_at?->format('d/m/Y H:i') }}</td></tr>
