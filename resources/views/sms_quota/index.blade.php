...
<style>
.saas-quota-payment-terms{display:flex;align-items:flex-start;gap:10px;margin-top:16px;padding:12px;text-align:left;border:1px solid rgba(255,98,110,.45);border-radius:11px;background:rgba(255,98,110,.08);color:var(--ds-text-secondary);font-size:.76rem;line-height:1.45}
.saas-quota-payment-terms input{width:18px;height:18px;flex:0 0 auto;margin-top:1px;accent-color:var(--ds-accent)}
.saas-quota-payment-terms strong{display:block;margin-bottom:3px;color:var(--ds-text-primary);font-size:.78rem}
@media(max-width:575px){.saas-quota-payment-terms{font-size:.74rem}}
</style>
@endpush
...
<section class="saas-card">
    ...
    <form id="quota-checkout-form">@csrf
        ...
        <label class="saas-quota-payment-terms" for="quota-payment-terms"><input id="quota-payment-terms" name="terms_accepted" type="checkbox" value="1" required><span><strong>Termes et conditions de paiement</strong>Je comprends que je serai redirigé vers la page de paiement KPrimePay. Les frais éventuels du moyen de paiement sont appliqués par l’opérateur et ne dépendent pas de MAXANOU.</span></label>
        <div class="saas-modal-actions"><button type="submit" class="saas-btn saas-btn-primary" id="quota-checkout-button" data-loading-text="Paiement en cours…" disabled><i class="bi bi-credit-card"></i> Payer avec KPrimePay</button></div>
    </form>
</section>
<section class="saas-card">
