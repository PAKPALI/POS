...
- Utiliser le composant global `window.ServerButtonLoader` décrit dans `docs/CAHIER_DES_CHARGES_DESIGN_SYSTEM_UI_UX.md`. Les formulaires ainsi que les appels jQuery/Fetch immédiats sont pris en charge automatiquement ; utiliser `withLoader` pour Axios, un Fetch différé ou un traitement asynchrone personnalisé.
